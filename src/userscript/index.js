...
function disableInlineWorker(reason) {
  ...
  console.warn('[Gemini-Eraser] Disable worker path:', reason);
...
async function processBlobWithBestPath(blob, options = {}) {
  if (workerClient) {
    try {
      ...
      console.warn('[Gemini-Eraser] Worker path failed, fallback to main thread:', error);
...
async function processImage(imgElement) {
  ...
  try {
    ...
    console.log('[Gemini-Eraser] Processed image');
  } catch (error) {
    ...
    if (retry.exhausted) {
      ...
      console.warn(
        `[Gemini-Eraser] Failed ${retry.failureCount} times, stop retrying to avoid resource leaks:`,
        error
      );
    } else {
      ...
      console.warn(
        `[Gemini-Eraser] Failed to process image, retry ${retry.failureCount}/${MAX_PROCESS_RETRIES} in ${retry.delayMs}ms:`,
        error
      );
    }
  } finally {
...
const processAllImages = () => {
  ...
  console.log(`[Gemini-Eraser] Found ${images.length} images to process`);
...
const setupMutationObserver = () => {
  ...
  console.log('[Gemini-Eraser] MutationObserver active');
};
...
unsafeWindow.fetch = async (...args) => {
  ...
  if (isGeminiGeneratedAssetUrl(url)) {
    console.log('[Gemini-Eraser] Intercepting:', url);
    ...
    try {
      ...
      console.warn('[Gemini-Eraser] Processing failed:', error);
      return response;
    }
  }
...
(async function init() {
  try {
    console.log('[Gemini-Eraser] Initializing...');
    if (canUseInlineWorker()) {
      try {
        workerClient = new InlineWorkerClient(USERSCRIPT_WORKER_CODE);
        console.log('[Gemini-Eraser] Worker acceleration enabled');
      } catch (workerError) {
        workerClient = null;
        console.warn('[Gemini-Eraser] Worker initialization failed, using main thread:', workerError);
      }
    }
    ...
    if (!workerClient) {
      ...
      getEngine().catch((error) => {
        console.warn('[Gemini-Eraser] Engine warmup failed:', error);
      });
    }
    ...
    console.log('[Gemini-Eraser] Ready');
  } catch (error) {
    console.error('[Gemini-Eraser] Initialization failed:', error);
  }
})();
