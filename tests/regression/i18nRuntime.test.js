...
test('should infer tr-TR locale from navigator language', async () => {
  setupRuntime({
    navigatorLanguage: 'tr',
    navigatorLanguages: ['tr', 'en-US'],
    fetchImpl: async () => ({
      ok: true,
      async json() {
        return { title: 'Deneme' };
...
test('t should replace arbitrary placeholders', async () => {
  setupRuntime({
    fetchImpl: async () => ({
      ok: true,
      async json() {
        return { message: 'Added {{count}} files' };
      },
    }),
  });
  ...
  i18n.translations.message = 'Added {{count}} files';

  assert.equal(i18n.t('message', { count: 3 }), 'Added 3 files');
});
